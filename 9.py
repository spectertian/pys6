...
class SaveDialog(QDialog):
    def __init__(self, pixmap, rect, parent=None):
        try:
            ...
            buttons_layout.addWidget(self.close_button, alignment=Qt.AlignTop)
            ...
            bottom_buttons_layout = QVBoxLayout(bottom_buttons_container)
            ...
            self.save_button.clicked.connect(self.save_and_keep_open)
            ...
            buttons_layout.addWidget(bottom_buttons_container, alignment=Qt.AlignBottom)
    ...
    def save_and_keep_open(self):
        ...
        # 通知父窗口重新加载截图列表
        if self.parent and hasattr(self.parent, 'load_saved_screenshots'):
            self.parent.load_saved_screenshots()

    def close(self):
        super().close()
        # 如果需要在关闭时执行其他操作，可以在这里添加
